...
def retrain():
  def generate():
    ...
    class command():
      def __init__(self):
        self.image_dir = os.path.join(folder1.get() , 'resize')
        self.output_graph = os.path.join(folder1.get() , 'model' , 'output_graph.pb')
        self.intermediate_output_graphs_dir = os.path.join(folder1.get() , 'tmp' , 'intermediate_graph')
        self.intermediate_store_frequency = 0
        self.output_labels = os.path.join(folder1.get() , 'model' , 'output_labels.txt')
        self.summaries_dir = os.path.join(folder1.get() , 'tmp' , 'retrain_logs')
        self.how_many_training_steps = int(steps_text.get())
        ...
        self.bottleneck_dir = os.path.join(folder1.get() , 'tmp' , 'bottleneck')
        self.final_tensor_name = 'final_result'
        self.flip_left_right = False
        self.random_crop = 0
        self.random_scale = 0
        self.random_brightness = 0
        self.tfhub_module = 'https://tfhub.dev/google/imagenet/inception_v3/feature_vector/1'
        self.saved_model_dir = ''

    FLAGS = command()
    tf.app.run(main=main, argv=[sys.argv[0]])
  
  def progress_retrain():
    pb_retrain.start()
    pb_retrain_label['text'] = "モデル生成中..."
  
  def retrain_thread():
    thread1 = threading.Thread(target=progress_retrain)
    thread1.start() 

    thread2 = threading.Thread(target=generate)
    thread2.start()   

  retrain = tk.Toplevel()
  # GUIの画面サイズ
  retrain.geometry('550x150')
  # 親ウインドウ操作不能にする
  retrain.grab_set()
  # GUIの画面タイトル
  retrain.title('学習モデル生成')
  # テキストボックス１
  steps_label = tk.Label(retrain, text='学習回数')
  steps_label.place(x=20, y=10)
  steps_text = tk.Entry(retrain)
  steps_text.insert(0, 500)
  steps_text.pack()
  steps_text.place(x=150, y=10)

  # 種類確認ボタン
  btn3 = tk.Button(retrain, text='生成開始', command=retrain_thread)
  btn3.pack()
  btn3.place(x=20, y=50)

  pb_retrain = ttk.Progressbar(retrain, mode="indeterminate",length=200)
  pb_retrain.pack()
  pb_retrain.place(x=20, y=100)

  pb_retrain_label = tk.Label(retrain, text='待機中')
  pb_retrain_label.pack()
  pb_retrain_label.place(x=250, y=100)
...
# 種類確認ボタン
btn3 = tk.Button(baseGround, text='学習モデルを生成', command=retrain)
btn3.pack()
btn3.place(x=330, y=120)
